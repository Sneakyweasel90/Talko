...
router.delete("/users/:id", requireAuth, requireAdmin, async (req, res) => {
  ...
  if (targetId === ownerId)
    return res.status(403).json({ error: "Cannot delete the server owner" });
  ...
  await db.query(
    `DELETE FROM local_nicknames WHERE owner_id = $1 OR target_id = $1`,
    [targetId],
  );
  ...
  await db.query(`DELETE FROM channel_last_read WHERE user_id = $1`, [
    targetId,
  ]);
  await db.query(`DELETE FROM dm_last_read WHERE user_id = $1`, [targetId]);
  await db.query(`DELETE FROM pinned_messages WHERE pinned_by = $1`, [
    targetId,
  ]);
  await db.query(
    `UPDATE messages SET username = '[deleted]', user_id = NULL WHERE user_id = $1`,
    [targetId],
  );
  await db.query(
    `DELETE FROM dm_conversations WHERE user1_id = $1 OR user2_id = $1`,
    [targetId],
  );
...
router.patch("/users/:id/role", requireAuth, requireAdmin, async (req, res) => {
  ...
  if (targetId === ownerId)
    return res
      .status(403)
      .json({ error: "Cannot change the server owner's role" });
  if (target.role === "admin" && req.user.id !== ownerId)
    return res
      .status(403)
      .json({ error: "Only the server owner can change another admin's role" });
...
router.post("/users/:id/kick", requireAuth, requireAdmin, async (req, res) => {
  ...
  await db.query(`UPDATE users SET kicked_until = $1 WHERE id = $2`, [
    kickedUntil,
    targetId,
  ]);
  await db.query(`DELETE FROM refresh_tokens WHERE user_id = $1`, [targetId]);
...
router.post("/users/:id/ban", requireAuth, requireAdmin, async (req, res) => {
  ...
  await db.query(`UPDATE users SET banned_at = NOW() WHERE id = $1`, [
    targetId,
  ]);
  await db.query(`DELETE FROM refresh_tokens WHERE user_id = $1`, [targetId]);
  forceDisconnectUser(targetId);
